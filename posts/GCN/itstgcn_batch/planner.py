...
class PLNR():
    def __init__(self,plans,loader,dataset_name=None,simulation_results=None):
        self.plans = plans
        col = ['dataset', 'method', 'mrate', 'mtype', 'lags', 'nof_filters', 'batch_size', 'inter_method', 'epoch', 'mse','calculation_time']
        self.loader = loader
    ...
    def simulate(self):
        for _ in range(self.plans['max_iteration']):  
            product_iterator = itertools.product(
                ...
                self.plans['batch_size'],
                self.plans['inter_method'],
                self.plans['epoch']
            )
            for prod_iter in product_iterator:
                method,mrate,lags,nof_filters,batch_size,inter_method,epoch = prod_iter
                ...
                t1 = time.time()
                lrnr.learn(filters=nof_filters,epoch=epoch, batch_size=batch_size)
                t2 = time.time()
                evtor = Evaluator(lrnr,train_dataset,test_dataset)
                evtor.calculate_mse()
                mse = evtor.mse['test']['total']
                calculation_time = t2-t1
                self.record(method,mrate,mtype,lags,nof_filters,batch_size,inter_method,epoch,mse,calculation_time)
